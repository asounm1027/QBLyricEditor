# 轻便歌词编辑器 (QBLyricEditor)

一个用于制作 / 编辑 LRC 格式歌词的轻量级 WPF 桌面应用（.NET 10）。支持导入音频与歌词、逐行打点计时、时间平移与微调、变速不变调播放以及预览。
...
- 导入音频与歌词（mp3 / wav / flac / m4a / aac 等音频格式，lrc / txt 歌词）
- 逐行打点计时，支持时间微调与整体 / 选中行的时间平移
- 变速不变调播放（NAudio + SoundTouch），音量与播放速度可调
- 微调步进预设（50 / 500 / 1k 毫秒）
