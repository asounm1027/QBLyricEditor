...
public class LrcLine : IComparable<LrcLine>
{
    ...
    public string LrcTimeText
        => LrcTime.HasValue ? LrcHelper.ToShortString(LrcTime.Value, IsShort) : string.Empty;
    ...
    public static LrcLine Parse(string line)
    {
        ...
        // 此时只能为正常歌词
        // 注意：只能按第一个 ] 分割，不能用 Split(']') 要求恰好两段，
        // 否则歌词正文本身含有 ] 字符时会被误判为格式错误
        var trimmed = line.TrimStart().TrimStart('[');
        int closeIndex = trimmed.IndexOf(']');
        if (closeIndex == -1) throw new FormatException();

        var tag = trimmed[..closeIndex];
        var content = trimmed[(closeIndex + 1)..];

        if (!LrcHelper.TryParseTimeSpan(tag, out TimeSpan time))
        {
            return new LrcLine(null, tag);
        }

        // 正常歌词和空白歌词不需要进行额外区分
        return new LrcLine(time, content);
